...
onready var obstacles = _get_obstacles_cells()
onready var _half_cell_size = cell_size / 2
onready var path_start_tile = tile_set.find_tile_by_name("path_start")
onready var path_end_tile = tile_set.find_tile_by_name("path_end")
...
func astar_add_walkable_cells(obstacle_list = []):
	...
	for y in range(map_size.y):
		for x in range(map_size.x):
			...
			if point in obstacle_list: #### SI NO ESTÁ EN LA LISTA NO SE PUEDE PASAR
				continue
